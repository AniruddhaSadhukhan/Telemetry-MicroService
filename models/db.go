...
func IsPersonalAccessTokenExist(ctx context.Context, filter interface{}) bool {

	count, err := GetDbSession().Database(config.Store.Database.TokenDBName).Collection(TokenCollection).CountDocuments(ctx, filter)
	if err != nil {
		logger.Log.Error("Error checking PAT existence: " + err.Error())
	}
	return count > 0
